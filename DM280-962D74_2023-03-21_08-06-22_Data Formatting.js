...
function onResult(decodeResults, readerProperties, output) {
    ...
    if (decodeResults[0].decoded) {
        ...
        if (decodeResults[0].symbology.name == "Data Matrix" || decodeResults[0].symbology.name == "QR") // is a DataMatrix or QR code?
        {
			...
			var cvtCenter = convertCenter(decodeResults[0].symbology.center);
            var center = cvtCenter.x * ratioScale + spacer + cvtCenter.y * ratioScale;
			//center = centerX +spacer + centerY;
            output.content = "\02" + decodeResults[0].content + spacer + center + spacer + convertAngle(angle)*ratioScale + "\03\n";
        }
        else {
            output.content = "READ ERROR\n";
        }
...
var inMin = 0, inMax = 360, outMin = -180, outMax = 0;
...
function convertCenter(curCenter) {
    ...
    if (curCenter.x >= centerX) {
        newCenter.x = curCenter.x - centerX;
    }
    else {
        newCenter.x = curCenter.x - centerX;
    }
	
	newCenter.y = centerY - curCenter.y;
